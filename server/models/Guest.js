...
export const addGuest = async (eventId, { guest_name, phone_number, guests_count }) => {
  const token = Math.random().toString(36).substring(2) + Date.now().toString(36);
  const [result] = await pool.query(
    'INSERT INTO guests (event_id, guest_name, phone_number, guests_count, invitation_token) VALUES (?, ?, ?, ?, ?)',
    [eventId, guest_name, phone_number || null, guests_count || 1, token]
  );
