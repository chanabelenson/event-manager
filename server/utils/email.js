...
export async function sendInvitationEmail({ guestEmail, guestName, invitationLink }) {
  try {
    const mailOptions = {
      from: `"מערכת ניהול אירועים" <${process.env.EMAIL_USER}>`,
      to: guestEmail,
      subject: `הזמנה לאירוע עבור ${guestName}`,
      html: `
        <div style="direction: rtl; font-family: Arial, sans-serif; text-align: center; max-width: 600px; margin: 0 auto; border: 1px solid #eee; padding: 20px; border-radius: 10px;">
          ...
          <p style="font-size: 16px; color: #555;">הוזמנת לאירוע שלנו! נשמח מאוד לראותך בין אורחינו.</p>
